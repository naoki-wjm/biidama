...
def test_first_publish_sends_everything_and_writes_ledger(tmp_path):
    ...
    assert sorted(res.sent) == ["a.html", "a/x.html", "index.html", "static/lightbox.js", "static/playlist.js", "static/style.css"]
...
def test_second_publish_sends_only_changes(tmp_path):
    ...
    ssh = FakeSSH()
    res = publish(cfg, REMOTE, runner=ssh, log=quiet)
    assert res.sent == ["a/x.html"] and res.unchanged == 5
...
def test_dry_run_sends_nothing_and_keeps_ledger(tmp_path):
    ...
    assert sorted(res.sent) == ["index.html", "static/lightbox.js", "static/playlist.js", "static/style.css"]
